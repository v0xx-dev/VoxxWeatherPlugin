...
{
    [HarmonyPatch]
    internal class HeatwaveTemperaturePatch
    {
        ...
        private static void HeatStrokePatchPrefix(PlayerControllerB __instance)
        {
            if (!((NetworkBehaviour)__instance).IsOwner || !__instance.isPlayerControlled)
                return;
            HeatwaveTemperaturePatch.prevSprintMeter = __instance.sprintMeter;
        }
        ...
        private static void HeatStrokePatchLatePostfix(PlayerControllerB __instance)
        {
            ...
            if (severity > 0)
            {
                float delta = __instance.sprintMeter - HeatwaveTemperaturePatch.prevSprintMeter;
                if (delta < 0.0) //Stamina consumed
                    __instance.sprintMeter = Mathf.Max(HeatwaveTemperaturePatch.prevSprintMeter + delta * (1 + severity * severityInfluenceMultiplier), 0.0f);
                else if (delta > 0.0) //Stamina regenerated
                    __instance.sprintMeter = Mathf.Min(HeatwaveTemperaturePatch.prevSprintMeter + delta / (1 + severity * severityInfluenceMultiplier), 1f);
            }
        }
    }
}
