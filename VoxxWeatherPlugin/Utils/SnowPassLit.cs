...
namespace VoxxWeatherPlugin.Utils
{
    public class SnowRenderersCustomPass : CustomPass
    {
        ...
        public float PCFKernelSize = 4.0f;
        public float snowNoisePower = 1f;
        public float snowScale = 100f;
        ...
        protected override void Setup(ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            {
                Debug.LogError("Depth render texture is not assigned.");
            }
        }
        ...
        protected override void Execute(CustomPassContext ctx)
        {
            ...
            if (overrideMaterial != null)
            {
                ...
                if (lightCamera != null)
                {
                    ...
                    ctx.propertyBlock.SetTexture("_DepthTex", depthRenderTexture);
                    ctx.propertyBlock.SetMatrix("_LightViewProjection", lightCamera.projectionMatrix * lightCamera.worldToCameraMatrix);
                    ctx.propertyBlock.SetFloat("_Bias", bias);
                    ctx.propertyBlock.SetVector("_LightDirection", lightCamera.transform.forward);
                    ctx.propertyBlock.SetFloat("_PCFKernelSize", PCFKernelSize);
                    ctx.propertyBlock.SetFloat("_SnowNoisePower", snowNoisePower);
                    ctx.propertyBlock.SetFloat("_SnowScale", snowScale);
                }
            }
